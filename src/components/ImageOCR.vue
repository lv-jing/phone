<template>
  <div class="image-ocr">
    <div class="container">
      ...
      <!-- Worker 预加载提示 -->
      <div v-if="!isWorkerReady && !isProcessing" class="worker-loading">
        <p>⏳ 正在初始化 OCR 引擎（首次使用需要下载模型文件，约 5-10MB）...</p>
        <div class="progress-bar">
          <div class="progress-fill" :style="{ width: workerLoadingProgress + '%' }"></div>
        </div>
        <p class="loading-hint">初始化完成后，后续识别会非常快速</p>
      </div>
...
const workerLoadingProgress = ref(0)
let imageFile = null
let worker = null // 复用 Worker，避免每次重新创建
...
// OCR错误修正：修正OCR识别时常见的错误（如 | 被识别为 1，或 1 被识别为 |）
const correctOCRText = (text) => {
  if (!text) return text
  
  // 常见OCR错误修正：在手机号上下文中，| 通常应该是 1
  // 匹配10位数字后跟 | 的情况（如 1595958000| 应该是 15959580001）
  let corrected = text.replace(/(\d{10})\|/g, '$11')
  
  // 匹配 | 前后都是数字的情况，将 | 替换为 1
  corrected = corrected.replace(/(\d)\|(\d)/g, '$11$2')
  
  // 匹配以 | 结尾且前面是10位数字的情况
  corrected = corrected.replace(/(1[3-9]\d{8})\|/g, '$11')
  
  // 匹配以 | 开头且后面是10位数字的情况（较少见，但也处理）
  corrected = corrected.replace(/\|(1[3-9]\d{9})/g, '1$1')
...
const extractPhoneNumbers = (text) => {
  // 使用 Map 存储手机号和其在原文中的位置，key为手机号，value为位置索引
  const phoneMap = new Map()
  
  // 先进行OCR错误修正，然后再清理文本
  const correctedText = correctOCRText(text)
  
  // 清理文本：将多个连续空格/换行/制表符替换为单个空格
  const cleanedText = correctedText.replace(/[\s\n\r\t]+/g, ' ').trim()
  console.log('清理后的文本（前500字符）:', cleanedText.substring(0, 500))
  console.log('完整文本长度:', cleanedText.length)
  
  // 辅助函数：记录手机号和位置
  const addPhone = (phone, position) => {
    // 先修正OCR错误（处理 | 被识别的情况）
    let correctedPhone = phone.replace(/\|/g, '1')
    
    // 只保留纯数字的手机号用于去重
    const digits = correctedPhone.replace(/\D/g, '')
    
    // 严格验证：11位且符合手机号格式
    if (digits.length === 11 && /^1[3-9]\d{9}$/.test(digits)) {
      // 如果已存在，只保留位置更靠前的（更早出现的）
      if (!phoneMap.has(digits) || phoneMap.get(digits) > position) {
        phoneMap.set(digits, position)
        console.log(`✓ 找到有效手机号: ${digits} (位置: ${position})`)
      }
    }
  }
  
  // 方法1: 使用边界匹配，确保手机号是独立的（前后不是数字）
  // 这是最准确的方法，优先使用
  const boundaryMobilePattern = /(?:^|[^\d])(1[3-9]\d{9})(?:[^\d]|$)/g
  let match
  while ((match = boundaryMobilePattern.exec(cleanedText)) !== null) {
    const phone = match[1]
    // match.index 是完整匹配的起始位置，需要找到手机号在匹配中的位置
    const phoneStartInMatch = match[0].indexOf(phone)
    const position = match.index + phoneStartInMatch
    addPhone(phone, position)
  }
  
  // 方法2: 匹配带分隔符的手机号格式（空格、横线、括号等）
  const mobilePatterns = [
    // 标准格式：138 0013 8000 或 138-0013-8000
    /1[3-9][\s\-\(\)]?\d{4}[\s\-\(\)]?\d{4}/g,
      ...
      if (digits.length === 11 && /^1[3-9]\d{9}$/.test(digits)) {
        ...
        for (let i = 0; i <= digits.length - 11; i++) {
          ...
          if (/^1[3-9]\d{9}$/.test(subDigits)) {
            // 计算在原文中的位置
            const position = match.index + i
            addPhone(subDigits, position)
            break // 找到一个就够了，避免重复
          }
        }
      }
  ...
  // 方法3: 处理10位数字+ | 的情况（如 1595958000| 应该是 15959580001）
  // 匹配10位数字后跟 | 的情况
  const tenDigitWithPipePattern = /(?:^|[^\d])(1[3-9]\d{8})\|(?:[^\d]|$)/g
  let pipeMatch
  while ((pipeMatch = tenDigitWithPipePattern.exec(cleanedText)) !== null) {
    const tenDigits = pipeMatch[1]
    const candidate = tenDigits + '1'
    if (/^1[3-9]\d{9}$/.test(candidate)) {
      const phoneStartInMatch = pipeMatch[0].indexOf(tenDigits)
      const position = pipeMatch.index + phoneStartInMatch
      addPhone(candidate, position)
      console.log(`修正10位数字+|: ${tenDigits}| -> ${candidate}`)
    }
  }
  
  // 方法4: 从连续数字中提取（处理OCR识别时数字连在一起的情况）
  // 匹配11位以上的连续数字序列
  const longNumberPattern = /\d{11,}/g
  let longMatch
  while ((longMatch = longNumberPattern.exec(cleanedText)) !== null) {
    const longDigits = longMatch[0]
    const startIndex = longMatch.index
    // 从长数字中滑动窗口提取11位手机号
  ...
  console.log('提取到的手机号（带位置信息）:', Array.from(phoneMap.entries()))
  
  // 按位置排序，然后转换为格式化后的手机号数组
  const result = Array.from(phoneMap.entries())
    .sort((a, b) => a[1] - b[1]) // 按位置排序
    .map(([phone]) => formatPhone(phone)) // 格式化
  
  console.log('格式化后的手机号（按原文顺序）:', result)
...
// 初始化 Worker（在页面加载时预加载，避免每次识别都重新创建）
const initWorker = async () => {
  if (worker) {
    return worker // 如果已存在，直接返回
  }
  ...
  try {
    ...
    console.log('开始预加载 Tesseract.js Worker...')
    
    // 创建 worker - CDN 版本的正确用法
    worker = await window.Tesseract.createWorker({
      logger: (m) => {
        console.log('Tesseract 预加载状态:', m.status, m.progress || 0)
        
        // 更新预加载进度
        if (m.status === 'loading tesseract core') {
          workerLoadingProgress.value = 10
        } else if (m.status === 'initializing tesseract') {
          workerLoadingProgress.value = 20
        } else if (m.status === 'loading language traineddata') {
          // 语言包加载是最耗时的，显示详细进度
          const p = m.progress || 0
          workerLoadingProgress.value = 30 + Math.round(p * 50)
        } else if (m.status === 'initializing api') {
          workerLoadingProgress.value = 85
        }
      }
    })
    ...
    workerLoadingProgress.value = 100
    
    console.log('✅ Tesseract.js Worker 预加载完成，可以开始识别了')
...
const extractText = async () => {
  ...
  // 如果 Worker 还没准备好，等待初始化完成
  if (!isWorkerReady.value || !worker) {
    error.value = 'OCR 引擎正在初始化，请稍候...'
    try {
      await initWorker()
    } catch (err) {
      error.value = 'OCR 引擎初始化失败: ' + (err.message || '未知错误')
      return
    }
  }
  ...
  try {
    ...
    // 使用 logger 更新识别进度
    const recognizeLogger = (m) => {
      if (m.status === 'recognizing text') {
        const p = m.progress || 0
        progress.value = 70 + Math.round(p * 25) // 从 70% 到 95%
      }
    }
    ...
    try {
      ...
      // 注意：这里不能直接传递 logger，因为 worker 已经创建好了
      // 如果需要显示识别进度，可以在创建 worker 时设置全局 logger
    ...
    console.log('识别完成（Worker 保持运行，下次识别会更快）')
    
    // 不再终止 worker，保持运行以便下次复用
...
// 组件挂载时预加载 Worker
onMounted(async () => {
  console.log('组件挂载，开始预加载 OCR Worker...')
  try {
    await initWorker()
  } catch (err) {
    console.error('Worker 预加载失败:', err)
    error.value = 'OCR 引擎初始化失败，请刷新页面重试'
  }
})
...
onUnmounted(async () => {
  if (worker) {
    console.log('组件卸载，清理 Worker...')
    try {
      await worker.terminate()
      worker = null
      isWorkerReady.value = false
    } catch (err) {
      console.error('清理 Worker 失败:', err)
    }
  }
})
</script>
